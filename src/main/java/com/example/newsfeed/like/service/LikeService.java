...
public class LikeService {
    ...
    public void postLike(User user, Long postId) {
        Post post = postRepository.findById(postId).    //입력한 postId 값으로 게시물이 있는지 찾고 없으면 오류 던짐
                orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "게시물이 없습니다."));

        if (post.getUser().getId().equals(user.getId())) { //게시물을 작성한 유저의 id와 로그인 되어있는 유저의 id가 같다면 좋아요 못누름.
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "본인의 게시글에는 좋아요를 누를 수 없습니다.");
        }

        Optional<Likes> found = likeRepository.findByPostIdAndStatus(post.getId(), Const.POST_LIKE);  //좋아요가 존재하는지 postId와 status 로 찾음.

        if (found.isPresent() && found.get().getStatus().equals(Const.POST_LIKE) && found.get().getPost().getId().equals(post.getId())) { // 이미 좋아요를 눌렀으며, 스테이터스가 '게시물 좋아요' 이고, 좋아요의 게시물 번호와 내가 입력한 게시물 번호가 같으면
            likeRepository.delete(found.get()); //좋아요 삭제
            likeRepository.flush(); //즉시 영속성 컨텐츠에 반영된 내용인 delete 를 데이터베이스에 적용
        } else { //게시글 좋아요를 누르지 않았다면
            Likes likes = new Likes(user, post);   //좋아요 생성자로 유저정보와 게시물 정보 주입
            likes.setStatus(Const.POST_LIKE);  //좋아요의 스테이터스를 게시글 좋아요로 저장
    ...
    public void commentLike(User user, Long commentId) {
        ...
        if (found.isPresent() && found.get().getStatus().equals(Const.COMMENT_LIKE) && found.get().getComment().getId().equals(comment.getId())) {  //좋아요를 이미 눌렀으며, 좋아요의 스테이터스가 '댓글 좋아요' 이고,좋아요의 댓글의 id가 내가 입력한 id와 같으면
            ...
            Likes likes = new Likes(user, comment.getPost(), comment); //좋아요 생성자의 파라미터로 누가 누른지 알기위해  user, 어느 게시물에 달렸는지 알기 위해 post, 어느 댓글인지 알기위해 comment 저장
